...
def debug(type_, value, tb):
  if hasattr(sys, 'ps1') or not sys.stderr.isatty():
    ...
    pdb.pm()
'''
def load_adjist(file_):
    count=0
    adjist=defaultdict(list)
    res = []
    with open(file_) as f:
        for l in f:
            ...
            node=row[0]
            neighbors = row[1:]
            adjist[node]=list(sorted(set(neighbors)))
    for v in adjist.keys():
      for other in adjist[v]:
        if v != other:
          adjist[other].append(v)
    for index in adjist.keys():
        if len(adjist[index])==0:
            res.append([index])
            count+=1
        while(len(adjist[index])>0):
            for i in adjist[index]:
                s = []
                s.append(i)
                adjist[index].remove(i)
                for j in adjist[index]:
                    if i in adjist[j]:
                        s.append(j)
                        adjist[index].remove(j)
                s.append(index)
                count+=len(s)
                res.append(s)
    return res,count
...
def load_edgelist(file_):
    ...
    with open(file_) as f:
        for l in f:
            s = l.strip().split()[:2]
            adjist[s[0]].append(s[1])
            adjist[s[1]].append(s[0])
    res = []
    for index in adjist.keys():
        while (len(adjist[index]) > 0):
            for i in adjist[index]:
