...
class Network(object):
    ...
    def SGD(self, training_data, epochs, mini_batch_size, eta,
        ...
        for j in range(epochs):
            ...
            print("Epoch %d, training complete" % (j,))
...
def split_data(labelfile_,nodefile_,embeddingfile_,len):
    ...
    train_inputs = [np.reshape(x, (64, 1)) for x in train_vec]
    train_results = [vectorized_result(y) for y in train_y]
    train_data = list(zip(train_inputs, train_results))
    test_inputs = [np.reshape(x, (64, 1)) for x in test_vec]
...
if __name__=='__main__':
    ...
    train_data,test_data=split_data(labelfile_,nodefile_,embeddingfile_,64)
    net=Network([64,60,20,10])
    net.SGD(train_data, 80, 10, 3.0, evaluation_data=test_data,monitor_evaluation_accuracy=True)